...
import './FullMath.sol';
/**
 * @dev solidity 0.8.0 breaks this contract - need to deploy from separate repo at ^0.7.0 || update math to ^0.8.0
 * /// contract commented out to allow compile
 *
 * /// @title DataStorage
 ...
 *   uint256 private constant UINT16_MODULO = 65_536;
 *
 *   struct Timepoint {
 ...
 *         int24 avgTick =
 *           int24(_getAverageTick(self, time, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative));
 *         int24 prevTick = tick;
 *         {
 ...
 *             prevTick =
 *               int24((last.tickCumulative - prevLast.tickCumulative) / (last.blockTimestamp - prevLast.blockTimestamp));
 *           }
 *         }
 ...
 *         (uint256(atOrAfter.secondsPerLiquidityCumulative - beforeOrAt.secondsPerLiquidityCumulative) * targetDelta)
 *           / timepointTimeDelta
 *       );
 ...
 *
 *     int24 avgTick =
 *       int24(_getAverageTick(self, blockTimestamp, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative));
 *     int24 prevTick = tick;
 *     if (index != oldestIndex) {
 *       Timepoint storage _prevLast = self[index - 1]; // considering index underflow
 ...
 *       prevTick =
 *         int24((last.tickCumulative - _prevLastTickCumulative) / (last.blockTimestamp - _prevLastBlockTimestamp));
 *     }
 *
 *     self[indexUpdated] =
 *       createNewTimepoint(last, blockTimestamp, tick, prevTick, liquidity, avgTick, volumePerLiquidity);
 *   }
 * }
 */
