...
abstract contract Common is Contracts, Params {
        ...
        if (address(collateralAuctionHouseFactory) != address(0)) {
            collateralAuctionHouse[ETH_A] = ICollateralAuctionHouse(
                collateralAuctionHouseFactory.deployCollateralAuctionHouse(
                    ETH_A,
                    _collateralAuctionHouseCParams[ETH_A]
                )
            );
        } else {
    ...
    function deployCollateralContracts(bytes32 _cType) public {
        ...
        if (address(collateralAuctionHouseFactory) != address(0)) {
            collateralAuctionHouse[_cType] = ICollateralAuctionHouse(
                collateralAuctionHouseFactory.deployCollateralAuctionHouse(
                    _cType,
                    _collateralAuctionHouseCParams[_cType]
                )
            );
        } else {
    ...
    function _setupContracts() internal {
        ...
        surplusAuctionHouse.modifyParameters(
            ...
            abi.encode(SURPLUS_AUCTION_BID_RECEIVER)
        );
    ...
    function _setupCollateral(bytes32 _cType) internal {
        ...
        oracleRelayer.initializeCollateralType(
            _cType,
            _oracleRelayerCParams[_cType]
        );
        liquidationEngine.initializeCollateralType(
            _cType,
            _liquidationEngineCParams[_cType]
        );

        taxCollector.initializeCollateralType(
            _cType,
            _taxCollectorCParams[_cType]
        );
        if (_taxCollectorSecondaryTaxReceiver.receiver != address(0)) {
            taxCollector.modifyParameters(
                _cType,
                "secondaryTaxReceiver",
                abi.encode(_taxCollectorSecondaryTaxReceiver)
            );
        }
