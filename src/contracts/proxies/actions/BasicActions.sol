...
contract BasicActions is CommonActions, IBasicActions {
  ...
  function _openSAFE(
    ...
    bytes32 _cType,
    address _usr
  ) internal returns (uint256 _safeId) {
  ...
  function _transferInternalCoins(
    address _manager,
    uint256 _safeId,
    address _dst,
    uint256 _rad
  ...
  function repayAllDebt(
    ...
    ISAFEEngine.SAFE memory _safeData = ISAFEEngine(_safeEngine).safes(
      _safeInfo.collateralType,
      _safeInfo.safeHandler
    );
