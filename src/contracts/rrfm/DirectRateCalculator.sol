...
contract DirectRateCalculator is SafeMath {
  ...
  constructor(uint256 acceleration_) public {
  ...
  function modifyParameters(bytes32 parameter, uint256 val) external isAuthority {
    if (parameter == 'acc') {
      ...
      revert('DirectRateCalculator/modify-unrecognized-param');
    }
  }
  ...
  function composeRate(int256 rateComposition) public pure returns (uint256) {
    return rateComposition >= 0
      ? addition(TWENTY_SEVEN_DECIMAL_NUMBER, uint256(rateComposition))
      : divide(
        multiply(TWENTY_SEVEN_DECIMAL_NUMBER, TWENTY_SEVEN_DECIMAL_NUMBER),
        addition(TWENTY_SEVEN_DECIMAL_NUMBER, uint256(-rateComposition))
      );
  }
  /*
  ...
  function decomposeRate(uint256 rawRate) public pure returns (int256) {
    return rawRate >= TWENTY_SEVEN_DECIMAL_NUMBER
      ? int256(subtract(rawRate, TWENTY_SEVEN_DECIMAL_NUMBER))
      : int256(TWENTY_SEVEN_DECIMAL_NUMBER)
        - int256(multiply(TWENTY_SEVEN_DECIMAL_NUMBER, TWENTY_SEVEN_DECIMAL_NUMBER) / rawRate);
  }
  ...
  function computeRate(
    ...
    return composeRate(
      decomposeRate(currentRedemptionRate)
        + (multiply(marketPrice, 10 ** 9) < redemptionPrice ? scaledProportional : -scaledProportional)
    );
  }
  /*
  ...
  function getScaledProportional(uint256 marketPrice, uint256 redemptionPrice) public view returns (int256) {
    ...
    uint256 proportionalTerm = (scaledMarketPrice <= redemptionPrice)
      ? subtract(redemptionPrice, scaledMarketPrice)
      : subtract(scaledMarketPrice, redemptionPrice);
