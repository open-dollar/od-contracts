...
contract PRawPerSecondCalculator is SafeMath, SignedSafeMath {
  ...
  constructor(
    ...
    require(
      ...
      'PRawPerSecondCalculator/invalid-sg'
    );
  ...
  function modifyParameters(bytes32 parameter, address addr) external isAuthority {
    if (parameter == 'seedProposer') {
      ...
      revert('PRawPerSecondCalculator/modify-unrecognized-param');
    }
  }
  /*
  ...
  function modifyParameters(bytes32 parameter, int256 val) external isAuthority {
    if (parameter == 'folb') {
      require(both(val < 0, val >= -int256(NEGATIVE_RATE_LIMIT)), 'PRawPerSecondCalculator/invalid-folb');
  ...
  /*
    ...
    */
  function getBoundedRedemptionRate(int256 pOutput) public view isReader returns (uint256, uint256) {
    int256 boundedPOutput = pOutput;
    uint256 newRedemptionRate;

    if (pOutput < feedbackOutputLowerBound) {
      boundedPOutput = feedbackOutputLowerBound;
    } else if (pOutput > int256(feedbackOutputUpperBound)) {
      boundedPOutput = int256(feedbackOutputUpperBound);
    }

    // newRedemptionRate cannot be lower than 10^0 (1) because of the way rpower is designed
    bool negativeOutputExceedsHundred = (boundedPOutput < 0 && -boundedPOutput >= int256(defaultRedemptionRate));
    ...
    if (negativeOutputExceedsHundred) {
      ...
      if (boundedPOutput < 0 && boundedPOutput <= -int256(NEGATIVE_RATE_LIMIT)) {
        ...
        newRedemptionRate = uint256(addition(int256(defaultRedemptionRate), boundedPOutput));
      }
    }
  ...
  function breaksNoiseBarrier(uint256 proportionalResult, uint256 redemptionPrice) public view isReader returns (bool) {
    uint256 deltaNoise = subtract(multiply(uint256(2), EIGHTEEN_DECIMAL_NUMBER), noiseBarrier);
    return proportionalResult
      >= subtract(divide(multiply(redemptionPrice, deltaNoise), EIGHTEEN_DECIMAL_NUMBER), redemptionPrice);
  }
  ...
  /*
    ...
    * @param redemptionPrice The system coin redemption price
    */
  function computeRate(uint256 marketPrice, uint256 redemptionPrice, uint256) external returns (uint256) {
    ...
    lastUpdateTime = now;
    // Multiply P by Kp
    proportionalTerm = multiply(proportionalTerm, int256(Kp)) / int256(EIGHTEEN_DECIMAL_NUMBER);
    // If the P * Kp output breaks the noise barrier, you can recompute a non null rate. Also make sure the output is not null
    if (breaksNoiseBarrier(absolute(proportionalTerm), redemptionPrice) && proportionalTerm != 0) {
      // Get the new redemption rate by taking into account the feedbackOutputUpperBound and feedbackOutputLowerBound
      (uint256 newRedemptionRate,) = getBoundedRedemptionRate(proportionalTerm);
  ...
  function getNextRedemptionRate(
    ...
    uint256
  ) public view isReader returns (uint256, int256, uint256) {
    // The proportional term is just redemption - market. Market is read as having 18 decimals so we multiply by 10**9
    // in order to have 27 decimals like the redemption price
    int256 rawProportionalTerm = subtract(int256(redemptionPrice), multiply(int256(marketPrice), int256(10 ** 9)));
    // Multiply P by Kp
    int256 gainProportionalTerm = multiply(rawProportionalTerm, int256(Kp)) / int256(EIGHTEEN_DECIMAL_NUMBER);
    // If the P * Kp output breaks the noise barrier, you can recompute a non null rate. Also make sure the output is not null
    if (breaksNoiseBarrier(absolute(gainProportionalTerm), redemptionPrice) && gainProportionalTerm != 0) {
      // Get the new redemption rate by taking into account the feedbackOutputUpperBound and feedbackOutputLowerBound
      (uint256 newRedemptionRate, uint256 rateTimeline) = getBoundedRedemptionRate(gainProportionalTerm);
  ...
  /*
    ...
    */
  function rt(uint256 marketPrice, uint256 redemptionPrice, uint256) external view isReader returns (uint256) {
  ...
  function ps() external view isReader returns (uint256) {
    return periodSize;
  }
