...
contract BasicPIRawPerSecondCalculator is SafeMath, SignedSafeMath {
  ...
  constructor(
    ...
    require(uint256(importedState[0]) <= now, 'BasicPIRawPerSecondCalculator/invalid-imported-time');
    require(
      both(Kp_ >= -int256(EIGHTEEN_DECIMAL_NUMBER), Kp_ <= int256(EIGHTEEN_DECIMAL_NUMBER)),
      'BasicPIRawPerSecondCalculator/invalid-sg'
    );
    require(
      both(Ki_ >= -int256(EIGHTEEN_DECIMAL_NUMBER), Ki_ <= int256(EIGHTEEN_DECIMAL_NUMBER)),
      'BasicPIRawPerSecondCalculator/invalid-ag'
    );
  ...
  function modifyParameters(bytes32 parameter, address addr) external isAuthority {
    if (parameter == 'seedProposer') {
      ...
      revert('BasicPIRawPerSecondCalculator/modify-unrecognized-param');
    }
  }
  /*
  ...
  function getBoundedRedemptionRate(int256 piOutput) public view isReader returns (uint256, uint256) {
    ...
    bool negativeOutputExceedsHundred = (piOutput < 0 && -piOutput >= int256(defaultRedemptionRate));
    ...
    if (negativeOutputExceedsHundred) {
      ...
      if (piOutput < 0 && piOutput <= -int256(NEGATIVE_RATE_LIMIT)) {
        ...
        newRedemptionRate = uint256(addition(int256(defaultRedemptionRate), piOutput));
      }
    }
  ...
  function computeRate(
    ...
    require(
      ...
      'BasicPIRawPerSecondCalculator/wait-more'
    );
