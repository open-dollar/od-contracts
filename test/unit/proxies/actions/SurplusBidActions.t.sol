...
contract SurplusBidActionTest is Test {
	...
	function test_increaseBidSize() public {
		...
		proxy.execute(address(surplusBidAuction), abi.encodeWithSignature('increaseBidSize(address,uint256,uint256)', _surplusAuctionHouse, _auctionId, _bidAmount));

		address savedDataSurplusAuctionHouse = _helperDecodeAsAddress(proxy.execute(target, abi.encodeWithSignature('surplusAuctionHouse()')));
	...
	function test_settleAuction() public {
		...
		proxy.execute(address(surplusBidAuction), abi.encodeWithSignature('settleAuction(address,address,uint256)', _coinJoin, _surplusAuctionHouse, _auctionId));
