...
/// @title TestProposal Script
/// @author OpenDollar
/// @notice Script without calldata
/// @dev NOTE This script requires the following env vars in the REQUIRED ENV VARS section below
/// @dev The script will propose no action, it is to test the process
...
/// @dev In the root, run: export FOUNDRY_PROFILE=governance && forge script --rpc-url <RPC_URL> script/testScripts/gov/ArbitraryAction/TestProposal.s.sol

contract TestProposal is JSONScript {
  function run() public {
    /// REQUIRED ENV VARS ///
    ...
    calldatas[0] = abi.encode('hello world');

    string memory description = 'hello world';
...

    {
      ...
      string memory objectKey = 'NO_ACTION_PROPOSAL';
      vm.serializeString(objectKey, 'action', 'no calldata');
