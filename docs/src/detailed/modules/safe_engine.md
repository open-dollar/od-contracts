...
The SAFE Engine serves as the central component of the OD framework, managing data on user-owned SAFEs (Simplified Agreement for Future Equity) and the interest rates for different forms of collateral. It performs the following functions:
...
- `safetyPrice`: The price of the collateral type (vs OD) at which the SAFE is considered unsafe.
- `liquidationPrice`: The price of the collateral type (vs OD) at which the SAFE is considered liquidatable.
...
### COIN, DEBT, and OD Dynamics

In this system, COINs and DEBT function similarly to matter and antimatter, created and annulled in pairs. The OD token acts as the ERC20 counterpart of a COIN when it's outside the system, maintaining a redeemable 1:1 ratio. The system enables users to lock COINs to mint OD or burn OD to unlock COINs, making them operable within the framework.
...
The "accumulated rate" represents the constantly accumulating interest or fees on the outstanding OD-generated debt. Calculated over time, it's based on the amount of OD minted and the applicable stability fee rate. This ensures that the debt owed by users evolves due to the ongoing addition of the stability fee. Whenever a user mints or repays OD, the formula for `generatedDebt` is:
...
>
> Initially, the `accumulatedRate` is 1. When Alice mints 100 OD tokens (COINs) from the SAFE Engine, her resulting debt is `100 * 1 = 100`.
>
> After one year, Alice repays her 100 debt. Now, the `accumulatedRate` stands at 1.1, requiring Alice to repay `100 * 1.1 = 110` OD tokens.
>
> Concurrently, Bob mints 100 OD, resulting in a (`100 / 1.1`) `90.9` debt.
>
> By year two, the `accumulatedRate` becomes 1.21. To repay his debt, Bob needs `90.9 * 1.21 = 110` OD tokens, identical to Alice's amount.
>
...
**Generating Debt and minting OD**:
...
**Notice**: Regular users may find that their `coinBalance` (in their proxy address) is usually `0` (or close to), as proxy interactions are designed to always use the `coinBalance` to mint OD and transfer the ERC20 to the user's address.
-->
